...
import icivics_pages.HP_2HeaderPlay;
import projectSpecific.base.ProjectSpecificMethods;

public class I32_HP_2Headerplay_Test extends ProjectSpecificMethods {
	...
	public void C8_Verifyplaysubmenu() throws IOException, InterruptedException {
		...
		new HP_2HeaderPlay(driver, node, prop, Environment, StageURL, Stage1URL).launchURL().verifyplaysubmenu();
	}
	...
	public void C9_Verifyplaybuttonlist() throws IOException, InterruptedException {
		...
		extent.attachReporter(spark);
		new HP_2HeaderPlay(driver, node, prop, Environment, StageURL, Stage1URL).launchURL().verifyplaysubmenu().verifyplaysubmenulist();
	}
	...
	public void C10_Verifyplaybuttonelements() throws IOException, InterruptedException {
		...
		extent.attachReporter(spark);
		new HP_2HeaderPlay(driver, node, prop, Environment, StageURL, Stage1URL).launchURL().Playbuttonelementsverification();
	}
	...
	public void C12_Verifypageisunderlined() throws IOException, InterruptedException {
		...
		extent.attachReporter(spark);
		new HP_2HeaderPlay(driver, node, prop, Environment, StageURL, Stage1URL).launchURL()
	...
	public void C13_Verifyclickonplaybuttonagain() throws IOException, InterruptedException {
		...
		extent.attachReporter(spark);
		new HP_2HeaderPlay(driver, node, prop, Environment, StageURL, Stage1URL).launchURL().verifyplaysubmenu().clickplaybuttonagain();
	}
}
