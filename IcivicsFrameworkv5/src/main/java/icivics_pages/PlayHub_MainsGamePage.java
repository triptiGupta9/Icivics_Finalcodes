...
public class PlayHub_MainsGamePage extends ProjectSpecificMethods {
	...
	public PlayHub_MainsGamePage(RemoteWebDriver driver, ExtentTest node, Properties prop, String Environment,
	...
	public PlayHub_MainsGamePage launchURL() {
	...
	public PlayHub_MainsGamePage Verifygamecarousel() {
	...
	public PlayHub_MainsGamePage Verifyleftcarousel() {
	...
	public PlayHub_MainsGamePage Verifyrightcarousel() {
	...
	public PlayHub_MainsGamePage Verifycarouseldots() {
		...
		for (int i = 0; i < number; i++) {
			...
			if (cssvalue1.equals(Yellowcolor)) {
				reportStep("Carousel dot is highlighted when clicked on '" + Gtitle + "' game dot" , "Pass");
			} else {
				reportStep("Carousel dot is not highlighted", "Fail");
			}
		}
	...
	public PlayHub_MainsGamePage Verifycarouseldots3rdgame() {
		...
		if (carouseldot3.isDisplayed()) {
			...
			reportStep("when clicking on the 3rd dot Carousel, 3rd game '" + GameTitle3 + "' is displayed", "Pass");
		} else {
			reportStep("Carousel is not available", "Fail");
		}
		return this;
	}

	@Given("Verify Banner to register as a student appears for anonymous user")
	public PlayHub_MainsGamePage Verifyregisterasastudentbanner() {
		WebElement registerbanner = propElement(getPropfile(gpropname1, "RegisterBanner"));
		WebElement registerbannertxt = propElement(getPropfile(gpropname1, "RegisterBannerTxt"));
		String bannertext = registerbannertxt.getText();
		waitTime(2000);
		if (registerbanner.isDisplayed()) {
			scrollToTheGivenWebElement(getPropfile(gpropname1, "RegisterBanner"));
			waitTime(2000);
			reportStep("'" + bannertext + "' , Banner with Text is displayed for anonymous user", "Pass");
			WebElement registerbannerbtn = propElement(getPropfile(gpropname1, "Registerbannerbutton"));
			reportStep("'" + registerbannerbtn.getText() + "' Button is displayed for anonymous user", "Pass");
		} else {
			reportStep("Banner to register as a student is not displayed for anonymous user", "Fail");
		}

		if (driver.getPageSource().contains("REGISTER AS A STUDENT")==false) {
			reportStep("Banner to 'register as a student' is displayed for anonymous user", "Pass");
			}
		else {
			reportStep("Banner to 'register as a student' is not displayed", "Fail");
		}
		
		return this;
	}

	@Given("Verify Banner to register as a student DOES NOT appear for logged in as student user")
	public PlayHub_MainsGamePage Verifyloginasastudentregisterasastudentbanner() {
		...
		if (driver.getPageSource().contains("REGISTER AS A STUDENT")==false) {
			reportStep("Banner to 'register as a student' is not displayed for logged in as student user", "Pass");
			}
		else {
			reportStep("Banner to 'register as a student' is displayed", "Fail");
		}
		return this;
	}

	@And("Verify Banner to register as a student DOES NOT appear for logged in as teacher user")
	public PlayHub_MainsGamePage Verifyloginasteacherregisterasastudentbanner() {
		String url = "/user/login";
		if (Environment.equals("Stage.d9")) {
			navigateto(StageURL + url);
		} else {
		...
		if (driver.getPageSource().contains("REGISTER AS A STUDENT")==false) {
			reportStep("Banner to 'register as a student' is not displayed for logged in as teacher user", "Pass");
			}
		else {
			reportStep("Banner to 'register as a student' is displayed", "Fail");
		}
	...
	public PlayHub_MainsGamePage Verifylregisterbannerbuttonclick() {
		...
		if (registerbannerbutton.isDisplayed()) {
			...
			String currenturl;
			if (Environment.equals("Stage.d9")) {
				currenturl = StageURL + url;
			} else {
				currenturl = Stage1URL + url;
			}

			if (driver.getCurrentUrl().equals(currenturl)) {
				reportStep(driver.getCurrentUrl() + " - redirected to Student registration Page", "Pass");
			}
			else {
				reportStep(driver.getCurrentUrl() + " - not redirected to Student registration Page", "Fail");
			}
		}
		return this;
	...
	public PlayHub_MainsGamePage Verifygamestitles() {
		List<WebElement> Tilelist = propElement1(getPropfile(gpropname1, "Tilelist"));
		List<WebElement> GameTitle = propElement1(getPropfile(gpropname2, "gameTitle"));
		List<WebElement> GameTime = propElement1(getPropfile(gpropname2, "gameTime"));
		List<WebElement> Gamesd = propElement1(getPropfile(gpropname2, "gamesd"));
		
		for (int i = 0; i < Tilelist.size(); i++){
			WebElement image = propElement(getPropfile(gpropname2, "gameurl" + i));
			String Title = GameTitle.get(i).getText();
			String Expectedplay = GameTime.get(i).getText();
			String ShortDesc = Gamesd.get(i).getText();
			String Tilename = Tilelist.get(i).getText();
			WebElement Tilebox = Tilelist.get(i);
			WebElement element = Tilebox;
			((JavascriptExecutor) driver).executeScript("arguments[0].scrollIntoView();", element);
			Actions builder = new Actions(driver);
			WebElement ele = propElement(getPropfile(gpropname2, "GameTile" + i));
			builder.moveToElement(ele).build().perform();
			if (image.isDisplayed()) {
				if (Tilename.contains("People's Pie")) {
					reportStep("People's Pie game Tile", "Fail");
					continue;
				} else { 
				reportStep("Image is displayed, "
						+ "Title of the game: " + Title + 
						", Expected play time: "+Expectedplay+
						", Short description: "+ ShortDesc + " are displayed", "Pass");
				}
			}
		}
		return this;
	...
	public PlayHub_MainsGamePage Verifygamestitlesclick() throws IOException {
	...
	public PlayHub_MainsGamePage Verifygreentilewithteacherlink() {
		...
		scrollToTheGivenWebElement(getPropfile(gpropname1, "Teachertile"));
		waitTime(3000);
		if (Teachertile.isDisplayed() && cssvalue.equals(greencolor)) {
			reportStep("Green Tile with teacher links is displayed after all the games", "Pass");
		} else {
	...
	public PlayHub_MainsGamePage Verifyteachbuttonclick() {
		...
		if (Teachbutton.isDisplayed()) {
			...
			reportStep("On clicking teach button it redirects to teacherspage successfully","Pass");
		} else {
			reportStep("On clicking teach button it will not redirects to teacherspage", "Fail");
		}
	...
	public PlayHub_MainsGamePage Verifygetstartedbuttonclick() {
		...
		if (getstartedbutton.isDisplayed()) {
			...
			reportStep("On clicking get started button it redirects to get started page successfully","Pass");
		} else {
			reportStep("On clicking get started button it is not redirects to get started page", "Fail");
		}
